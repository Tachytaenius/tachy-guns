...
function onProjItemCheckMovement(projectile)
	...
	if previousExhaustion then
		-- Please note that it uses a compromise system where the total exhaustion change per tick is multiplied down (or up), so it doesn't actually account specifically for exhaustion from firing (which depends on attributes) and would multiply a unit's exhaustion from running too if running while shooting
