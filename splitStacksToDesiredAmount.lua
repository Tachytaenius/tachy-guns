...
local function splitStacksToDesiredAmount(job)
	...
	local splitAnyStacks = customRawTokens.getToken(reaction, "SPLIT_STACKS_TO_DESIRED_AMOUNT")
	if not splitAnyStacks then return end
	...
	(function(...)
		for i = 1, select("#", ...) do
			...
			item:splitStack(item.stack_size - reagent.quantity, true)
		end
...
return splitStacksToDesiredAmount
